...
export class ChatContainerComponent implements OnInit {
    ...
    public openChat(user: User): void {
        ...
        let chat = this.store.chats
            .filter(chat => chat.participants.some(p => p === user.id))
            .sort(this.store.sortByLastUpdate)

        if(chat && chat.length) {
            this.currentChat = chat[0];
            this.currentChat.messages = [];
            this.pubnub.getHistory(this.currentChat.channel).then((response) => {
                this.currentChat.messages = response.messages.map(item => item.entry);
            });
        }
        else {
            this.startChat(user);
    ...
    public sendMessage(text: string): void {
        ...
        let update: Chat = Object.assign({}, { ...this.currentChat }, { last_update: this.http.getUTCTimestamp() });
        this.http.updateChat(update);
    }
    ...
    public addContact(id: string) {
        let prev = this.store.saved.map(i => i.id);
        this.http.addContact(this.store.currentUser.id, [...prev, id]);
    }
